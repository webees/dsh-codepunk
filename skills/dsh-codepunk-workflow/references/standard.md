...
| D088 | 模型统一与回退机制：子代理全链路继承主进程模型（删除岗位 agentOptions 自声明）；三级回退链 bai/qwen3.8-flash → llm-deepseek/deepseek-v4-flash → mtplx 本地；持续失败（≥3 次）run-lead 裁决切换（harness 无原生 fallback，流程层实现） | references/model-fallback.md / settings agent-default-model |
