...
| D096 | 持久 shell 与工具调用超时：接 `dsh-tool-bash-persistent`（与 `tool-bash` 同名共存，官方 minimal 同款写法；`timeoutMs` 300000；残留 TOOL_TIMEOUT 由调用方判超时重试或人工介入）；`dsh-tool-call-timeout-policy` 全局生效不开单独配置——凡声明 `timeoutMs` 的工具超时自动返回 TOOL_TIMEOUT，run-lead 见之按 brief 重试或拆小任务（不静默丢）| SKILL §4 / agent.cordis.yml |
