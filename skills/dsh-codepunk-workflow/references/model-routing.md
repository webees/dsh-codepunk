...
1. 三席实现子代理与评审子代理启动即失败，报错为「Model '<裸名>' is not available.
   Supported models: <带前缀的型号名列表>」—— 上游已改用**带前缀的型号名**，
   而本地 `models:` 目录里登记的是**裸名**（两种命名口径不一致）。
2. 该 baseURL 为**本地代理端点**；同一端点的另一 provider 已按前缀名登记，可作旁证。
