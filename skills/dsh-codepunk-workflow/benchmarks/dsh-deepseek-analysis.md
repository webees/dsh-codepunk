...
- **渠道与工变更**：`web_search` 工具因 API key 无效**全部失败**（`Authentication Fails`）→ 依任务预案转 **curl 直连**，全部渠道逐条验证 https 200 后取数：
  ...
  5. **本地源码**：`git clone --depth 1 https://github.com/deepseek-ai/deepseek-harness.git`（master @ 2026-08-25）到 `/tmp/dsh-repo` 后 grep/read —— 作为源码级一手事实源。注意：本地 DSH Desktop 的 `app.asar.unpacked` 是桌面外壳（anywhere-labs/deepseek-harness-desktop），**不是**主仓库，仅用于对照外壳组装，未作为组件事实源。
