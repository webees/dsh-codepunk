...
# --scan 扫描根：默认只含总库根（仅标记不迁移）；工程目录由调用方用 --scan-roots 或
...
# 命中这些名字的工程只迁流程资产，固件/镜像等大素材默认排除
ASSETS_AUTO_EXCLUDE=("__ASSETS_HEAVY__")
...
用法:
  ...
  dsh-codepunk-migrate.sh --migrate <src> <project_id> [选项]
     ...
     选项:
       --exclude-assets   跳过 *.ipsw / *.dmg.aea / *.dmg / *.app / >100M（素材重项目自动启用）
...
cmd_migrate() {
  ...
  # 默认排除：素材重项目（显式 --exclude-assets 已启用则仅提示）
