...
# 扫描根：默认 $HOME/Desktop（macOS 常见位置），可用 SCAN_ROOT 覆盖
SCAN_ROOT="${SCAN_ROOT:-${DESKTOP:-$HOME/Desktop}}"
QUIET=0
[ "${2:-}" = "--quiet" ] && QUIET=1
...
# --- 0. 环境前提 ----------------------------------------------------------
[ -d "$SCAN_ROOT" ] || fatal "Desktop 目录不存在: $SCAN_ROOT"
...
say "-- [1/2] 扫描 $SCAN_ROOT 根下散落 worktree --"
SCATTER=0
